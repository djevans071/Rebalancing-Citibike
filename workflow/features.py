...
def new_features(df):
    ...
    # turn strings 'True' and 'False' into booleans
    ...
    df = pd.merge(df, total_docks, how = 'outer', on = 'date').rename(columns = {'tot_docks_y': 'tot_docks'})
    df = df.drop('tot_docks_x', 1)
    ...
    # make new categorical variable: flux_type
    # if pct_flux > 0.2 ---> flux_type 1 (rebalance down -- remove bikes)
    # if pct_flux < -0.2 ---> flux_type -1 (rebalance up -- add bikes)
    # if abs(pct_flux) <= 0.2 ---> flux_type 0 (don't rebalance)
    df['flux_type'] = df.pct_flux.apply(flux_conditions).astype('category')
    df['temp'] = df.temp.apply(temp_conditions).astype('category')
    df['precip'] = df.precip.apply(precip_conditions).astype('category')

    # get lagged features
    df_with_lags = make_lagged_fluxes(df)
